...
export default async function AlbumPhotosPage({ params, searchParams }: Props) {
  ...
  const { data: album, error: albumError } = await supabase.from("albums").select("id,title,slug").eq("id", albumId).single();
  if (albumError || !album) notFound();
  const { data, error } = await supabase.from("photos").select("id,title,description,image_url,thumbnail_url,flickr_page_url,sort_order,is_published").eq("album_id", albumId).order("sort_order", { ascending: true });
    ...
    <div className="mx-auto max-w-7xl py-10">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <form action={createPhotosBulk} className="rounded-3xl border bg-white/90 p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          <button className="mt-4 rounded-xl bg-neutral-900 px-5 py-3 font-semibold text-white dark:bg-white dark:text-black">Thêm tất cả URL</button>
        </form>
      ...
      <section className="mt-12"><div className="mb-5 flex items-end justify-between"><div><p className="text-sm uppercase tracking-[.25em] text-neutral-500">Thư viện</p><h2 className="text-3xl font-bold">{photos.length} ảnh</h2></div></div><PhotoManager albumId={albumId} photos={photos}/></section>
    </div>
  </main>;
}
